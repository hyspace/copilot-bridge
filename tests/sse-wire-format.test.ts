...
describe("Responses SSE wire compatibility", () => {
  test("bounded queues do not greedily drain an unread event stream", async () => {
    let pulls = 0
    let cancelled = false
    ...
    const stream = normalizeResponsesSseStream(input)
  ...
  test("rejects an oversized unterminated frame instead of retaining it forever", async () => {
    const input = new ReadableStream<Uint8Array>({
      start(controller) { controller.enqueue(encoder.encode("data:" + "x".repeat(8 * 1024 * 1024))) },
    })
    await expect(new Response(normalizeResponsesSseStream(input)).text()).rejects.toThrow("character limit")
  })
